...
if __name__ == "__main__":
    ...
    isExist = os.path.exists(os.path.join(save_path,"results_poses_TEASER"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_TEASER"))
    
    #check if results_poses_TEASER/results directory exists
    isExist = os.path.exists(os.path.join(save_path,"results_poses_TEASER/results"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_TEASER/results"))

    #check if results_poses_TEASER/ply directory exists
    isExist = os.path.exists(os.path.join(save_path,"results_poses_TEASER/ply"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_TEASER/ply"))
    ...
    for i in tqdm(iterator):
        ...
        write_results_to_txt(os.path.join(save_path,"results_poses_TEASER/results/obj_{}_result_{}.txt".format(obj_id, i)), obj_id,  ir, correspondences, add_score, score, HybridPose_add_score, adds_score, add_score_ICP, add_score_ICP, HybridPose_add_score_ICP, adds_score_ICP,
                      T_gt, T_est, reg_p2p.transformation, error_cm, error_deg)
        
        # Write ply files

        #check if results_poses_TEASER/ply/i directory exists
        isExist = os.path.exists(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}".format(obj_id, i)))
        if not isExist:
            os.makedirs(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}".format(obj_id, i)))

        # CAD coordinates
        cad_03d = np_2_o3d(CAD_ver)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}/cad_{}.ply".format(obj_id, i, i)), cad_03d)
        ...
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}/cad_{}_pose_est.ply".format(obj_id, i, i)), cad_o3d_transformed_est)

        # CAD coordinates transformed (GROUND TRUTH)
        cad_transformed_gt = transform(CAD_ver, T_gt)
        cad_o3d_transformed_gt = np_2_o3d(cad_transformed_gt)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}/cad_{}_pose_gt.ply".format(obj_id, i, i)), cad_o3d_transformed_gt)

        # PC coordinates
        pc_03d = np_2_o3d(PC_ver)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_TEASER/ply/obj_{}_result_{}/pc_{}.ply".format(obj_id, i, i)), pc_03d)

# Append results to metric specific lists and write average results to txt file
with open(os.path.join(save_path,'results_poses_TEASER/avg_results.txt'), 'w') as txt_file:
