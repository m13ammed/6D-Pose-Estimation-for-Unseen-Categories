...
def replace_idx_by_coo(idx, coo):
    ...
    if len(result_list) == 0 : return np.array([])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="A simple script to process two paths.")
    parser.add_argument("directory_path", help="The first path.")
    parser.add_argument("save_path", help="The second path.")
    args = parser.parse_args()

    directory_path = args.directory_path #'/data/unseen_object_data/tmp/results/results_lm_pbr_document/'
    ...
    isExist = os.path.exists(os.path.join(save_path,"results_poses_RANSAC"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_RANSAC"))
    
    #check if results_poses_RANSAC/results directory exists
    isExist = os.path.exists(os.path.join(save_path,"results_poses_RANSAC/results"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_RANSAC/results"))

    #check if results_poses_RANSAC/ply directory exists
    isExist = os.path.exists(os.path.join(save_path,"results_poses_RANSAC/ply"))
    if not isExist:
        os.makedirs(os.path.join(save_path,"results_poses_RANSAC/ply"))
    ...
    for i in tqdm(iterator):
        ...
        if src_T.ndim ==0 or dst_T.ndim == 0: continue
        ...
        write_results_to_txt(os.path.join(save_path,"results_poses_RANSAC/results/obj_{}_result_{}.txt".format(obj_id, i)), obj_id,  ir, correspondences, add_score, score, HybridPose_add_score, adds_score, add_score_ICP, add_score_ICP, HybridPose_add_score_ICP, adds_score_ICP,
                      T_gt, T_est, reg_p2p.transformation, error_cm, error_deg)
        
        # Write ply files
        #check if results_poses_RANSAC/ply/i directory exists
        isExist = os.path.exists(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}".format(obj_id, i)))
        if not isExist:
            os.makedirs(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}".format(obj_id, i)))

        # CAD coordinates
        cad_03d = np_2_o3d(CAD_ver)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}/cad_{}.ply".format(obj_id, i, i)), cad_03d)
        ...
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}/cad_{}_pose_est.ply".format(obj_id, i, i)), cad_o3d_transformed_est)

        # CAD coordinates transformed (GROUND TRUTH)
        cad_transformed_gt = transform(CAD_ver, T_gt)
        cad_o3d_transformed_gt = np_2_o3d(cad_transformed_gt)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}/cad_{}_pose_gt.ply".format(obj_id, i, i)), cad_o3d_transformed_gt)

        # PC coordinates
        pc_03d = np_2_o3d(PC_ver)
        o3d.io.write_point_cloud(os.path.join(save_path,"results_poses_RANSAC/ply/obj_{}_result_{}/pc_{}.ply".format(obj_id, i, i)), pc_03d)

with open(os.path.join(save_path,'results_poses_RANSAC/avg_results.txt'), 'w') as txt_file:
