...
from DPFM.dpfm.utils import get_mask, nn_interpolate
    ...
    def forward(self, coords0, coords1, features_x, features_y, batch):
        ...
        for layer in self.layers:
            ...
            if self.cross_sampling_ratio == 1:
                ...
                torch.cuda.empty_cache()
                desc0 = nn_interpolate(sampled_desc0.transpose(1, 2), flat_coords0.t(), torch.Tensor(dists0), torch.Tensor(idn0).type(torch.int), torch.Tensor(idf0)).transpose(1, 2)
                desc1 = sampled_desc1#nn_interpolate(sampled_desc1.transpose(1, 2), flat_coords1.t(), dists1, idn1, idf1).transpose(1, 2)
